...
class OTPPage extends StatefulWidget {
  ...
  const OTPPage({super.key, required this.nomor});
...
class _OTPPageState extends State<OTPPage> {
  ...
  Future<void> resendOTP() async {
    ...
    final response = await http.post(
      Uri.parse('http://localhost/flutter_otp_payment/otp_forgot_password.php'),
    ...
    if (_otp.length == 6) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (responseBody.contains('otp benar')) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('OTP berhasil diverifikasi!')),
            );
            Navigator.pushReplacementNamed(context, '/login');
          } else if (responseBody.contains('otp expired')) {
            ScaffoldMessenger.of(context).showSnackBar(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Terjadi kesalahan saat verifikasi, coba lagi.')),
        );
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Image.asset('assets/image/otp and payment/waiting.png', height: 200),
